...
/obj/item/dildo
	name 				= "dildo"
	desc 				= "Floppy!"
	icon 				= 'modular_skyrat/icons/obj/genitals/dildo.dmi'
	force 				= 0
...
/obj/item/dildo/AltClick(mob/living/user)
	. = ..()
	if(!istype(user) || !in_range(user, src) || user.incapacitated())
		return
	customize(user)
	return TRUE
...
/obj/item/dildo/examine(mob/user)
	. = ..()
	if(can_customize)
		to_chat(user, "<span class='notice'>Alt-Click [src] to customize it.</span>")
